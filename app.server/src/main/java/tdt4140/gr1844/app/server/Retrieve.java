...
class Retrieve {
    ...
    static JSONObject listFeelings(int patientID, String orderBy, String cookie) throws ClassNotFoundException, SQLException, InstantiationException, IllegalAccessException {
        ...
        if ((Authentication.isAuthenticated(cookie, "doctor") && Authentication.isDoctorsPatient(cookie, patientID)) || Authentication.isDataOwner(patientID, cookie) || (Authentication.isAuthenticated(cookie, "admin"))) {
            SQL sql = new SQL();
            String query;
